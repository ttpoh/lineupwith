...
<?php
                ...
                $number = $_GET['number'];
                $bno = $_GET['board_number'];
                session_start();
                $query = "select title, content, date, hit, id from review where number =$number";
                
                
                                                             
                $hit = "update review set hit=hit+1 where number=$number";
                $db->query($hit);
        ...
        <div class="view_btn">
            <button
                class="view_btn1"
                onclick="location.href='update.php?number=<?=$number?>&id=<?=$_SESSION['userid']?>'">수정</button>
            <button class="view_btn1" onclick="location.href='boardM.php'">목록으로</button>
            <button
                class="view_btn1"
		...
		?>
            <div class="dap_lo">
                ...
                <div class="rep_me dap_to" style="float:right"><?php echo $reply['date']; ?>                   
                    ...
                    <a class="dat_edit_bt" href="repU.php?number=<?php echo $number?>&board_number=<?php echo $reply['number']?>">수정</a>                    
                    <a class="dat_delete_bt" href="repD.php?number=<?php echo $number?>&board_number=<?php echo $reply['number']?>">삭제</a>
					</div>                   
            ...
            <!--- 댓글 입력 폼 -->
            <div class="dap_ins">
                <form action="viewR.php?number=<?php echo $number; ?>" method="post">
                    <input
                        type="text"
                        name="dat_user"
                        id="dat_user"
